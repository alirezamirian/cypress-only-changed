...
export function runFixture(fixture: string, changedFiles: string[]): string[] {
  ...
  try {
    ...
    if ((e.status ?? -1) > 1) {
      throw new Error(
        `cypress exited ${e.status ?? 'unknown'}\n${e.stdout ?? ''}\n${e.stderr ?? ''}`,
      );
    }
  }
...
export function assertRan(ran: string[], expectedToRun: string[], expectedToSkip: string[]): void {
  for (const spec of expectedToRun) {
    assert.ok(ran.includes(spec), `expected "${spec}" to run — got: [${ran.join(', ')}]`);
  }
  for (const spec of expectedToSkip) {
    assert.ok(!ran.includes(spec), `expected "${spec}" to be skipped — got: [${ran.join(', ')}]`);
  }
}
